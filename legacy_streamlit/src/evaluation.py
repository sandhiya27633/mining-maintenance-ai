...
def compute_scenario_metrics(
    ...
    for tag in ["NORMAL", "DISRUPTION", "ALL"]:
        if tag == "ALL":
            ...
            p_sub = proto_sim[proto_sim.get("scenario_tag", pd.Series(["NORMAL"]*len(proto_sim))) == tag] \
                if "scenario_tag" in proto_sim.columns \
                else proto_sim[proto_sim["scenario_tag"] == tag]
            b_sub = baseline_sim[baseline_sim["scenario_tag"] == tag] \
        ...
        results.append(ScenarioMetrics(
            ...
            breakdowns_avoided=avoided,
            reduction_pct=red,
            prototype_prevented=int(p_sub["sim_breakdown_prevented"].sum()),
            baseline_prevented=int(b_sub["sim_breakdown_prevented"].sum()),
...
def compute_sensitivity_analysis(
    ...
    for config_name, weights in cfg.DCSS_WEIGHT_CONFIGS.items():
        ...
        # Baseline (same for all configs)
        base_sim = simulate_breakdowns(
            apply_baseline(features_df), svc_state,
...
def _build_notes(report: EvaluationReport) -> List[str]:
    ...
    if not report.target_met:
        notes.append(
            ...
            "Root cause: With the synthetic dataset's 30-day service cycle and the seeded RNG, "
            "both models prevent most breakdown events — the differential is small in absolute terms. "
            "The prototype's variable intervals (3/18/30/45 days) shift timing more than count. "
            "Proposed adjustment: increase FAILURE_BETA_INTERCEPT reduction or extend simulation "
            "to 2 years to widen the gap. Data NOT altered to force the number (Section 10.4)."
        )
    else:
        notes.append(

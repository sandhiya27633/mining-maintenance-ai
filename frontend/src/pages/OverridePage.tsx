...
export default function OverridePage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([v, p, h]) => {
        ...
        if (!selVehicle && vehicleData.length > 0) {
          setSelVehicle(vehicleData[0].id)
        }

        /*
         * If a planId was passed in the URL and that plan exists,
         * select it directly.
         */
        if (planId) {
          const urlPlan = planData.find((p: any) => p.id === planId)

          if (urlPlan) {
            setSelPlan(urlPlan.id)
            setNewInterval(urlPlan.recommended_interval_days)
          }
        }
      })
      .catch(() => {
  ...
  /*
   * Show plans belonging to the selected vehicle.
   *
   * IMPORTANT:
   * We do not blindly hide overridden plans here.
   * Instead, we prefer a non-overridden plan when one exists.
   */
  const vehiclePlans = plans.filter(
    (p: any) => p.vehicle_id === selVehicle
  )

  const availablePlans = vehiclePlans.filter(
    (p: any) => !p.is_overridden
  )

  /*
   * If the backend currently returns an already-overridden latest plan,
   * keep it visible rather than showing an empty dropdown.
   *
   * This is mainly useful for the current production data.
   */
  const plansForVehicle =
    availablePlans.length > 0
      ? availablePlans
      : vehiclePlans

  /*
   * Sort newest plan first.
   */
  const sortedPlans = [...plansForVehicle].sort((a: any, b: any) => {
  ...
  useEffect(() => {
    ...
    /*
     * Keep the currently selected plan if it still exists.
     */
    const currentPlanExists = sortedPlans.some(
      (p: any) => p.id === selPlan
    )
    ...
    if (currentPlanExists) {
      ...
      if (currentPlan) {
        setNewInterval(currentPlan.recommended_interval_days)
      }
    ...
    /*
     * Otherwise automatically select the newest plan.
     */
    const newestPlan = sortedPlans[0]

    setSelPlan(newestPlan.id)
    setNewInterval(newestPlan.recommended_interval_days)
  }, [selVehicle, plans, selPlan])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedPlan) {
      setError('Selected maintenance plan could not be found.')
      return
    }
    ...
    try {
      ...
      setSuccess(
        `Override submitted. New interval: ${newInterval} days. This action is audited.`
      )

      setReason('')
      setDispatcherId('')

      const [updatedPlans, updatedHistory] = await Promise.all([
        maintenanceAPI.recommendations(),
        maintenanceAPI.overrideHistory(),
      ])
      ...
      <div className="card">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={e => {
                ...
                setSelPlan(selectedId)

                const selected = sortedPlans.find(
                  (p: any) => p.id === selectedId
                )

                if (selected) {
                  setNewInterval(
                    selected.recommended_interval_days
                  )
                }
              }}
              required
            >
              ...
              {sortedPlans.map((p: any) => (
                <option key={p.id} value={p.id}>
                  {p.plan_date}
                  {' — DCSS '}
                  {typeof p.dcss_score === 'number'
                    ? p.dcss_score.toFixed(1)
                    : p.dcss_score}
                  {' ['}
                  {p.risk_level}
                  {'] — '}
                  {p.recommended_interval_days}d
                  {p.is_overridden ? ' — overridden' : ''}
                </option>
              ))}
            </select>

            {sortedPlans.length === 0 && (
              <p className="text-risk-critical text-xs mt-2">
                No maintenance plan found for this vehicle.
                Run DCSS Analysis first.
              </p>
            )}
          </div>
          ...
          <div>
            <label className="label">
              Reason
              <span className="text-risk-critical ml-1">
                * required
              </span>
            </label>
            ...
            {reason.trim().length === 0 &&
              reason.length > 0 && (
                <p className="text-risk-critical text-xs mt-1">
                ...
                {overrideHistory.map((ov: any) => (
                  <tr key={ov.id}>
                    ...
                    <td className="font-mono">
                      {ov.original_interval_days}d
                    </td>

                    <td className="font-mono">
                      <span
                        className={
                          ov.overridden_interval_days <
                          ov.original_interval_days
                            ? 'text-risk-high'
                            : 'text-risk-low'
                        }
                      >

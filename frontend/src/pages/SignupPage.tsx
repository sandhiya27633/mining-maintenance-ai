...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    // Create backend profile (with JWT from new session)
    if (data.session) {
      try {
        await authAPI.createProfile(displayName || undefined)
      } catch {
        // Profile may auto-create via DB trigger — non-fatal
      }
    }
    ...
    <div className="min-h-screen bg-surface flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        <div className="card">
          ...
          <p className="text-sm text-gray-400 mb-6">Start managing your fleet with duty-cycle AI</p>
          ...
          <p className="mt-5 text-center text-sm text-gray-400">
            ...
            <Link to="/login" className="text-accent-blue hover:underline">Log in</Link>
          </p>
        </div>
      </div>
    </div>

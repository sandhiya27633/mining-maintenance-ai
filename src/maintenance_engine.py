...
def compute_interval(risk_level: str) -> tuple[int, str, str]:
    ...
    if risk_level == "CRITICAL":
        ...
        # Use the configured reduction factor, but add a conservative
        # minimum safety cadence for high-risk mining vehicles.  This keeps
        # the recommendation condition-based rather than calendar-only.
        configured_days = int(
            round(std * cfg.HIGH_INTERVAL_REDUCTION_FACTOR)
        )

        # High-risk vehicles must not be allowed to drift back toward the
        # normal 30-day cadence merely because of rounding/configuration.
        high_risk_cap = max(
            cfg.MIN_INTERVAL_DAYS,
            int(round(std * 0.50)),
        )

        days = min(configured_days, high_risk_cap)
        days = max(
            cfg.MIN_INTERVAL_DAYS,
            min(cfg.MAX_INTERVAL_DAYS, days),
        )
        ...
        reason = (
            ...
            f"high-risk cadence is capped at {high_risk_cap} days "
            f"for harsh-duty operation."
        )
...
def apply_recommendations(df: pd.DataFrame) -> pd.DataFrame:
    ...
    required_columns = {"risk_level", "date", "dcss", "vehicle_id"}
    missing = required_columns.difference(df.columns)
    if missing:
        raise ValueError(
            "Maintenance engine is missing required columns: "
            + ", ".join(sorted(missing))
        )
